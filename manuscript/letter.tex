...
\section{Anomalous quantities in gravity modelling}
...
geophysics. At point $P$, it is defined as the difference between the 
gravity vector at a point $Q^{\prime}$ on the Geoid (a particular 
equipotential surface of the gravity potential) and the normal gravity 
vector at a point $Q$ on the ellipsoid surface, both at the same
geodetic latitude and longitude (Fig. \ref{fig:surfaces}) as in
\begin{equation}
\Delta \mitbf{g}_{P} = \mitbf{g}_{Q^{\prime} } - \mitbf{\gamma}_{Q} .
\label{eq:gravity-anomaly-vector}
\end{equation}
Similarly as the gravity disturbance expression (eq.
...
However, many authors in the literature have. All these authors have
...
the gravity disturbance $\delta g_{i}$ (eq. \ref{eq:gravity-disturbance-approx})
...
\section{Comparing Free-air anomaly with gravity disturbance}

By definition, for a small height $h$, the normal gravity at point
$P(h, \varphi, \lambda)$ can be expanded into a Taylor series 
in terms of $h$ \citep{heiskanen-moritz1967,
hofmann-wellenhof-moritz2005} as in
\begin{equation}
\gamma_{P} \approx \gamma_{Q} + \frac{\partial \gamma}{\partial h} h ,
\label{eq:gamma-expansion}
\end{equation}
where higher orders of $h$ were neglected. $h$ represents the
distance between points $P$ and $Q$ shown in (Fig. \ref{fig:heights}).
Therefore, substituting \ref{eq:gamma-expansion}'s result into
eq. \ref{eq:gravity-disturbance}, the gravity disturbance becomes
\begin{equation}
\delta g_{P} \approx g_{P} - (\gamma_{Q} + \frac{\partial \gamma}{\partial h} h) .
\label{eq:disturb-approx}
\end{equation}
According to \citet{heiskanen-moritz1967, blakely1996}, the relation
between gravity on the geoid and on the reference ellipsoid is
a similar Taylor expansion given by
\begin{equation}
g_{Q^{\prime}} = g_{P} - \frac{\partial \gamma}{\partial h} H
\label{eq:geod2phys_surf}
\end{equation}
where $H$ is defined as the orthometric height (Fig. \ref{fig:heights})
measured along the curved plumb line, starting from the Geoid
\citep{heiskanen-moritz1967, hofmann-wellenhof-moritz2005}.
Thus, the Free-air anomaly $(\Delta g^{a})$, which corrects for the
decrease in gravity due to the elevation of the observation point $P$, 
can be written as
\begin{equation}
\Delta g^{a}_{P} \approx g_{P} - (\gamma_{Q} + \frac{\partial \gamma}{\partial h} H)
\label{eq:free-air}
\end{equation}
By inspection, one easily concludes that the difference is the first
order derivative term
\begin{equation}
\delta g_{P} - \Delta g^{a}_{P} \approx - \frac{\partial \gamma}{\partial h} (h-H) = - \frac{\partial \gamma}{\partial h} N ,
\label{eq:disturb-FA}
\end{equation}
where $N$ is the geoidal undulation (Fig. \ref{fig:heights}) and 
$\frac{\partial \gamma}{\partial h} = -0.3086$.  Hence, using a
mean value of $\tilde{N} = 120$m, the absolute value of the
difference in eq. \ref{eq:disturb-FA} is computed
\begin{equation}
\parallel \delta g_{P} - \Delta g^{a}_{P} \parallel \leq \parallel - \frac{\partial \gamma}{\partial h} \tilde{N} \parallel = 37.032
\label{eq:diff-constant}
\end{equation}
...
the gravity data approximate the gravitational disturbance.
...
Hence, almost all forward modelling techniques compute 
...
the terminology changes, the authors must bear in mind the 
...
\begin{figure}
    ...
    \caption{Schematic representation of the gravity vector
        ...
        gravity disturbance vector $\delta \mitbf{g}_{P}$
        (eq. \ref{eq:gravity-disturbance-vector}) and unit vector 
        $\hat{\mitbf{u}}_{P}$ (eq. \ref{eq:unit-vectors}) at a point $P$ on the surface of the Earth; gravity vector 
        $\mitbf{g}_{Q^{\prime}}$ at a point $Q^{\prime}$ on the Geoid;
        normal gravity vector $\mitbf{\gamma}_{Q}$ at a point $Q$ on
        the reference ellipsoid. The dashed line passing through
...
\begin{figure}
    \includegraphics{figures/different-heights.png}
    \caption{Schematic representation of the geometric height
        $h$, orthometric height $H$ and geoidal undulation $N$.
        $H$ is curved because it follows the plumbline.}
  \label{fig:heights}
\end{figure}
